...
/**
 * Move constructor */
s21::S21Matrix::S21Matrix(S21Matrix &&other) noexcept : rows_(0), cols_(0), matrix_(nullptr) {
  ...
  if (rows_ == 1) {
	...
	for (auto i = 0; i < rows_; ++i) {
	  for (auto j = 0; j < cols_; ++j) {
		...
		//		result(i, j) = sign * result(i, j); // скорее всего решение выше лучше
	  }
	}
  }
  return result;
...
double s21::S21Matrix::Determinant() {
  ...
  if (rows_ == 1) {
	...
	if (!check) result *= sign;// проверь всегда ли верный знак получается, мб условие не нужно
  }
  return result;
}
...
void s21::S21Matrix::GetMiniMatr_(const int &x, const int &y, S21Matrix *result) {
  int di = 0, dj = 0;
